...
def assessment_plotter(delays, args):
    ...
    ax3.plot(keys,np.sqrt(delay_times_l2),marker='o', linestyle='-',label='diff')
    ...
    # ax4.set_ylabel('RMS')
    ax3.legend()
    ax0.set_title(f'sample = {args.sample_name}, run = {args.run_number}, qmin = {args.q_min_assess:.2f}, qmax = {args.q_max_assess:.2f}')
