...
def main():
    ...
    if args.normalize_to_target_id is None:
        midpoint = (max(delay) + min(delay))/2.
        args.normalize_to_target_id = find_nearest(delay, midpoint)
    target_key = delay[args.normalize_to_target_id]
    morph_cfg = morph.morph_default_config(scale=args.initial_scale,stretch=args.initial_stretch,
    ...
    for delay_t, data in raw_delays.items():
        morphed = morph.morph(data[0], data[1], data[0], target[1], rmin=args.q_min_normalize, rmax=args.q_max_normalize, **morph_cfg) #on
        _, on_morph, _, _ = morphed['morph_chain'].xyallout
        morphed = morph.morph(data[0], data[2], data[0], target[1], rmin=args.q_min_normalize, rmax=args.q_max_normalize, **morph_cfg) #off
